...
for line in lines:
    ...
    if len(line.split()) > 1:
        ...
        url, author_given, rules_given = line, "", ""
    if not bool(url_check.match(url)):
        pack_title = line
        continue
    real_url, title, author, rules, img, source = get_image_and_rules(url)
    if author == "":
        author = author_given
    if rules == "":
        rules = rules_given
    details.append([url, real_url, title, author, rules, img, source])
